...
CFL = .125 # relative size of a time-step
T   = .25 # final time
...
# @. VX[iids] += .1/K1D*randn()
# @. VY[iids] += .1/K1D*randn()
VX = @. VX + .25*sin(pi*VX)*sin(pi*VY)
VY = @. VY + .25*sin(pi*VX)*sin(pi*VY)
...
function rhs(u, rd::RefElemData, md::MeshData)
    ...
    udu   = u.*(dudx+dudy) # (u*dudx + u*dudy,v)
...
for i = 1:Nsteps
    for INTRK = 1:5
        ...
        if INTRK==5
            rhstest[i] = sum(u.*(J.*(M*rhsQ))) # (du/dt,u) + [(RHS(u), u)?=0]
        end
