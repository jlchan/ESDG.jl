...
function build_node_maps(xf,yf,zf,Nfaces,EToE,EToF)
    ...
    for k1=1:K
        for f1=1:Nfaces
            ...
            if (k1==k2)
                ...
                x1 = x1*onevec;  y1 = y1*onevec;  z1 = z1*onevec
                x2 = x2*onevec;  y2 = y2*onevec;  z2 = z2*onevec

                # Compute distance matrix
                D = abs.(x1 - transpose(x2)) .+ abs.(y1-transpose(y2)) .+ abs.(z1-transpose(z2))
                refd = maximum(D[:])
                idM = map(x->x[1], findall(@. D < NODETOL*refd))
                idP = map(x->x[2], findall(@. D < NODETOL*refd))
                mapP[idM,f1,k1] = @. idP + (f2-1)*Nfp + (k2-1)*Nfaces*Nfp
