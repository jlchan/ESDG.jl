...
using Basis1D
theme(:wong)
gr(aspect_ratio=1,legend=nothing,axis=nothing,border=:none,ticks=nothing,
...
plot!([-1;1].+3/4,[1;-1].+2/4,seriescolor=:black,lw=3,ls=:dash,label = nothing)
plot!([-1;1].+3/4,[-1;-1].+2/4,seriescolor=:black,lw=3,ls=:dash,label = nothing)
scatter!(r_tri.+3/4,s_tri.+2/4,markersize=5,markercolor=:white,markerstrokecolor=:black,markerstrokewidth=2)
scatter!(rf.+3/4,sf.+2/4,markersize=3,markershape=:rect,markercolor=:white,markerstrokecolor=:black,markerstrokewidth=2)
...
scatter!([r_tri[4]+3/4:3/4:3],[s_tri[4]+2/4:2/4:2],markersize=5,markercolor=:royalblue1,markerstrokecolor=:black,markerstrokewidth=2)
plot!([r_tri[4]+3/4,r_tri[4]+3],[s_tri[4]+2/4,s_tri[4]+2],seriescolor=:royalblue1,lw=3,ls=:dash)

scatter!([r_tri[2].+(3/4:3/4:3)],[s_tri[2].+(2/4:2/4:2)],markersize=5,markercolor=:royalblue1,markerstrokecolor=:black,markerstrokewidth=2)
plot!([r_tri[2]+3/4,r_tri[2]+3],[s_tri[2]+2/4,s_tri[2]+2],seriescolor=:royalblue1,lw=3,ls=:dash)
savefig("reference_fourier_GPU_1.png")
