...
const h = minimum(minimum(md.J,dims=1)./maximum(md.sJ,dims=1))

# Make domain periodic
@unpack Nfaces,Vf = rd
@unpack xf,yf,K,mapM,mapP,mapB = md
LX,LY = (x->maximum(x)-minimum(x)).((VX,VY)) # find lengths of domain
mapPB = build_periodic_boundary_maps(xf,yf,LX,LY,Nfaces*K,mapM,mapP,mapB) # only in x coordinate
mapP[mapB] = mapPB
@pack! md = mapP

# idLeft = findall(@. abs(xf[:]+1)<1e-8)
# idRight = mapP[idLeft]
# error("d")
...
function initial_condition() # ρ,u,v,p
    Ma = .3
    return 1,.1,0, 1/(Ma^2*γ)
end
U = ntuple(a->zeros(size(x)),4)
for field = 1:4
    U[field] .= initial_condition()[field]
end
# U[1] .= @. 2 + .5*sin(pi*x)*sin(pi*y)
...
function dg_grad(Q,Qf,QP,md::MeshData,rd::RefElemData)
    ...
    rhsx = volx.(Qr,Qs) .+ surf.(QP,Qf,ntuple(x->nxJ,4))
    rhsy = voly.(Qr,Qs) .+ surf.(QP,Qf,ntuple(x->nyJ,4))
...
function init_BC_funs(md::MeshData)
    ...
    topwall      = mapB[findall(@. abs(yb-1) < 1e-12)]
    ...
    function impose_BCs_entropyvars!(VUP,VUf,md::MeshData)
        ...
        Uxwall = @. 0*exp(-5*xtopwall^2)
        V2wall = @. -Uxwall / VUf[4][topwall]
        VUP[2][topwall] .= 2*V2wall - VUf[2][topwall]
    end
...
function visc_rhs(Q,md::MeshData,rd::RefElemData)
    ...
    dV[1] .= 0 # ignore penalty for mass equation - slightly fewer DOPRI steps
    # tau = .1
